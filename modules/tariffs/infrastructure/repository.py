...
def save(schedule: TariffSchedule) -> None:
    ...
    Грубо, но честно, и выбрано осознанно. Сопоставить версию со строкой
    можно было бы по дате начала действия — но именно её и правит `correct`,
    а до шага C1b две версии вправе иметь одну дату. Сопоставление по дате
    молча потеряло бы дубликат; перезапись целиком возвращает в таблицу ровно
    то, что в линии, включая дубликаты.
