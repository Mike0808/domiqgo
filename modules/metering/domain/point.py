...
class PeriodClosed(Exception):
    """Период объявлен закрытым: показания в нём не создаются и не меняются.

    Замок ставит Billing в момент выставления счёта
    ([ADR-0012](../../../docs/architecture/adr/0012-metering-period-lock.md)):
    предъявленный документ не должен расходиться с данными, из которых
    получен. Хранит и применяет замок Metering — иначе пришлось бы спрашивать
    Billing, а это замкнуло бы цикл: обращение Billing → Metering уже занято
    ради потребления.
    """


def ensure_period_open(period, closed_periods) -> None:
    """Отказать, если период закрыт.

    Правило здесь, а не в командах, по той же причине, что и остальные:
    проверяться оно должно без базы. Сегодня закрытых периодов не бывает —
    ставить замок некому до шага E4, — но входов, которые обязаны его
    соблюдать, уже три: портал, админка и бот.
    """
    if period in closed_periods:
        raise PeriodClosed(
            f"Период {period:%m.%Y} закрыт: счёт за него уже выставлен. "
            "Чтобы внести правку, снимите замок периода.")


class ReadingNotFound(LookupError):
    """За этот период по этому ресурсу показание не вносилось.

    Исправляют то, что уже сдано; всё остальное — сдача, а это другая
    операция с другим смыслом и другим событием.
    """
